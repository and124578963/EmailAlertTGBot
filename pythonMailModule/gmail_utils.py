...
class MailUtil:
    ...
    def get_mail_ids(self, folder: str, mail_connection):
        ...
        mail_ids = ids.split()
        logging.debug(f"Letters IDs gathered. Details: {mail_ids}")
        return mail_ids
    ...
    def check_html_structure(self, dict_result):
        ...
        if (text.find("<html") != -1 and text.find("</html>") != -1) or text.find("<br>") != -1:
    ...
    def chek_subjects_in_db(self, list_mails_data, chek_in_db):
        ...
        # print(chek_in_db)
        # print("Вход:" + str(list_mails_data))
        ...
        # print("перед проверкой:" + str(subject_dict))
        if chek_in_db == 'Subjects':
            ...
            list_mails_data = list(filter(lambda mail_, conn=collection_name:
                                          self.chek_id_db_for_filter(conn, mail_), list_mails_data))
            # print("чек текст:" + str(list_mails_data))
            list_result = []
            for mail in list_mails_data:
                dict_result = {
                    'mail_id':  self.conf['EMAIL_FOLDER'] + mail["mail_id"],
                    'key': subject_dict[mail['subject']]['key'],
                    'subject': '',
                    'text': mail['body'],
                    'attachments': self.save_attachment(mail['raw_data'], download_folder=ATTACHMENTS_FOLDER),
                    'sended': 0,
                    'assign': 1 if self.conf['ENABLE_ASSIGN_TO_PEOPLE_IN_CHAT'] else 0,
                    'topic': self.conf['EMAIL_FOLDER']
    ...
    def chek_id_db_for_filter(self, connection, body):
        mail_id = self.conf['EMAIL_FOLDER'] + body.get('mail_id')

        # Проверяем наличие письма в БД по id
        item_details = connection.find({"mail_id": mail_id})
        try:
            item_details.next()
        except StopIteration:
            return True
    ...
    def main(self):
        ...
        try:
            ...
            offset_collection.insert_one({"_id": self.conf['EMAIL_FOLDER'],
                'folder': self.conf['EMAIL_FOLDER'],
                                          'offset': 0
                                          })
        ...
        end = str(time_now + datetime.timedelta(**PERIOD_OF_SCAN_LETTER))[:19]
        ...
        if self.conf['ONLY_FIRST_MAIL']:
            list_mails_data = list(filter(lambda x:
                                          ...
                                          list_mails_data))
        # Исключаем письма с ненужными темами
        if self.conf.get('RESTRICTED_SUBJECTS', "None") != 'None':
            list_mails_data = list(filter(lambda mail:
                                          all([mail['subject'].lower().find(i.lower()) == -1 for i in self.conf['RESTRICTED_SUBJECTS']]),
                                          list_mails_data))
        ...
        try:
            if len(list_result) > 0:
                ...
                print("Записи не добавлены, оффсет обновлен")
